...
function DeviceList({ devices, selectedDeviceId, setSelectedDeviceId, setOpenDropdown, onDeleteDevice, onCheckPermissions }: any) {
    ...
    return (
        <div className="flex flex-col h-full bg-black/40 p-4 border border-white/5 rounded-3xl mt-4">
            <div className="flex items-center justify-between mb-4">
                <h3 className="text-sm font-bold text-fg-2 flex items-center gap-2">
                    <Smartphone size={16} className="text-accent" /> Your Devices
                </h3>
                {devices.length > 0 && (
                    <button 
                        ...
                        className={`text-[10px] font-bold uppercase tracking-wider px-3 py-1.5 rounded-lg transition-all ${
                            isSelectionMode 
                                ...
                                : 'bg-white/5 text-fg-3 hover:bg-white/10 border border-transparent hover:border-white/10'
                        }`}
                    >
                                    ...
                                    <div className="flex items-center gap-2">
                                        {!isSelectionMode && (
                                            <div 
                                                onClick={(e) => {
                                                    e.stopPropagation();
                                                    if (device.online && onCheckPermissions) onCheckPermissions(device);
                                                }}
                                                className={`flex items-center justify-center p-2 rounded-xl transition-all ${
                                                    device.online 
                                                        ? 'bg-white/5 hover:bg-white/10 text-zinc-400 hover:text-white cursor-pointer border border-transparent hover:border-white/20' 
                                                        : 'bg-white/[0.02] text-zinc-600 cursor-not-allowed opacity-50'
                                                }`}
                                                title={device.online ? "Check Permissions" : "Device Offline"}
                                            >
                                                <Settings size={18} />
                                            </div>
                                        )}
...
interface AppNavigationProps {
    ...
    onCheckPermissions?: (device: any) => void;
    user?: any;
...
export default function AppNavigation({ 
    ...
    userPlan, setShowPlansModal, handleSignOut, onOpenAppModal, onDeleteDevice, onCheckPermissions, user,
                    ...
                    <div className="relative">
                        ...
                        <AnimatePresence>
                            {openDropdown === 'devices' && (
                                ...
                                >
                                    <DeviceList devices={devices} selectedDeviceId={selectedDeviceId} setSelectedDeviceId={setSelectedDeviceId} setOpenDropdown={setOpenDropdown} onDeleteDevice={onDeleteDevice} onCheckPermissions={onCheckPermissions} />
                                </motion.div>
                            )}
                        </AnimatePresence>
                    </div>
                                ...
                                >
                                        <div className="p-4 mb-3 rounded-2xl neo-surface flex flex-col items-center text-center border border-white/10 shadow-lg">
                                            <div className={`w-16 h-16 rounded-full mb-3 flex items-center justify-center overflow-hidden ring-2 ${
                                                userPlan === 'enterprise' ? 'ring-purple-500 shadow-[0_0_25px_rgba(147,51,234,0.7)]' :
