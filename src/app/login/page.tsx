...
export default function LoginPage() {
    ...
    return (
        <main className="min-h-screen bg-[#080807] text-[#fafaf9] overflow-x-hidden selection:bg-amber-200/20 pt-12">
            ...
            {/* ═══ EXTRA CAPABILITIES (Desktop/PC only so mobile reaches Login Portal instantly) ═══ */}
            <section id="capabilities" className="hidden lg:block relative z-10 py-32 px-5 bg-gradient-to-b from-transparent via-white/[0.015] to-transparent border-y border-white/[0.04]">
                ...
                <Reveal delay={0.15} className="w-full max-w-5xl mx-auto">
                    <div className="premium-card-border shadow-[0_30px_100px_rgba(0,0,0,0.9)]">
                        <div className="premium-card p-8 sm:p-14 md:p-16 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center bg-gradient-to-br from-black/80 via-[#12110f]/90 to-black/90">
                            ...
                            {/* Left Side: Grand Architectural Invitation & Security Trust (Desktop/PC only E.g. hidden on mobile so users directly hit the login form) */}
            ...
            {/* ═══ INSTANT LOGIN MODAL OVERLAY (OPENS ON TOP WHEN GET STARTED FOR FREE IS CLICKED) ═══ */}
            <AnimatePresence>
                {showLoginModal && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/85 backdrop-blur-xl animate-fade-in">
                        <motion.div
                            ...
                            className="relative w-full max-w-xl rounded-[2.2rem] border border-white/20 bg-gradient-to-br from-[#161412] via-[#0f0e0d] to-black p-6 sm:p-10 shadow-[0_30px_100px_rgba(0,0,0,0.95)] overflow-hidden max-h-[90vh] overflow-y-auto"
                        >
                            {/* Ambient glow */}
                            ...
                            <button
                                ...
                                className="absolute top-5 right-5 w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 text-zinc-300 hover:text-white flex items-center justify-center transition-all z-20"
                            >
                            ...
                            <div className="mb-6 relative z-10">
                                <div className="flex items-center gap-3.5 mb-4">
                                    <div className="w-12 h-12 rounded-xl overflow-hidden ring-1 ring-amber-400/40 shadow-md">
                                    ...
                                    <div>
                                        ...
                                        <span className="text-[11px] text-emerald-400 font-mono font-semibold flex items-center gap-1.5 mt-0.5">
                                            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" /> v2.6 Enterprise Session Active
                                        </span>
                                    </div>
                                </div>
                                <p className="text-xs sm:text-sm text-zinc-300 font-light">Enter your authorized credentials below or authenticate instantly via Google OAuth.</p>
                            </div>

                            <div className="relative z-10">
                                <button type="button" onClick={() => signIn('google', { callbackUrl: '/' })} className="premium-btn-google py-3.5 text-sm sm:text-base font-bold shadow-lg w-full">
                                    <svg width="20" height="20" viewBox="0 0 24 24"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/></svg>
                                    <span>Continue with Google</span>
                                </button>
                                ...
                                <div className="flex items-center gap-4 my-5">
                                    <div className="flex-1 h-px bg-white/[0.08]" />
                                    <span className="text-[10px] text-zinc-500 uppercase tracking-widest font-bold">or use email credentials</span>
                                    <div className="flex-1 h-px bg-white/[0.08]" />
                                </div>

                                <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                                    <div>
                                        <label htmlFor="modal-email-input" className="block text-xs font-bold uppercase tracking-wider text-zinc-300 mb-1.5">Email Address</label>
                                        <input id="modal-email-input" type="email" value={email} onChange={(e) => { setEmail(e.target.value); setError(''); }} placeholder="executive@domain.com" required autoComplete="email" className="premium-input py-3 text-sm sm:text-base" />
                                    </div>
                                    <div>
                                        <div className="flex items-center justify-between mb-1.5">
                                            <label htmlFor="modal-password-input" className="block text-xs font-bold uppercase tracking-wider text-zinc-300">Password</label>
                                            <span className="text-xs text-[#d4a574] hover:underline cursor-pointer font-medium">Forgot?</span>
                                        </div>
                                        <input id="modal-password-input" type="password" value={password} onChange={(e) => { setPassword(e.target.value); setError(''); }} placeholder="••••••••••••••••" required autoComplete="current-password" className="premium-input py-3 text-sm sm:text-base" />
                                    </div>
                                    ...
                                    <button type="submit" disabled={isLoading} className="premium-btn-primary py-3.5 mt-1 text-sm sm:text-base font-extrabold shadow-xl w-full">
                                        {isLoading ? (
                                            <><svg className="animate-spin h-4 w-4 inline mr-2" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z"/></svg>Authenticating Session…</>
                                        ) : 'Sign In to Console'}
                                    </button>
                                </form>
                            </div>
