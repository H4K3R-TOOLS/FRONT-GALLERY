            ...
            {show && (
                ...
                >
                    <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
                        ...
                        <button onClick={() => onScrollTo('demo-video')} className="px-2.5 py-1 rounded-full text-[11px] sm:text-xs font-bold text-zinc-300 hover:text-white hover:bg-white/10 transition-all hidden sm:inline-block">
                            Demo
                        </button>
                    </div>
...
export default function LoginPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (isSignUp) {
            try {
                ...
                if (!res.ok) {
                    setIsLoading(false);
                    if (data.error === "GOOGLE_ACCOUNT_ONLY") {
                        setShowGoogleAlert(true);
                    } else {
                ...
                if (r?.ok) {
                    ...
                    if (r?.error === "GOOGLE_ACCOUNT_ONLY" || String(r?.error).includes("GOOGLE")) {
                        ...
                        // Account created but auto-login failed — show clear sign-in prompt
                        setIsSignUp(false);
                        setError('');
                    }
                }
    ...
    return (
        <main className="min-h-screen bg-[#080807] text-[#fafaf9] overflow-x-hidden selection:bg-amber-200/20 pt-8">
                            ...
                            <div className="lg:col-span-6 hidden lg:flex flex-col justify-between h-full space-y-8 pr-0 lg:pr-6 border-b lg:border-b-0 lg:border-r border-white/10 pb-10 lg:pb-0">
                                <div>
                                    ...
                                    <h4 className="text-xl sm:text-2xl font-bold text-white mb-4 leading-snug">
                                        Why top creators and administrators choose Spynox:
                                    </h4>
                                    <ul className="space-y-4 text-sm text-zinc-300 font-light">
                                        <li className="flex items-start gap-3">
                                            <span className="w-6 h-6 rounded-full bg-orange-500/20 border border-orange-500/40 flex items-center justify-center text-orange-400 flex-shrink-0 mt-0.5 text-xs font-bold">✓</span>
            ...
            <AnimatePresence>
                {showLoginModal && (
                    <motion.div
                        ...
                        className="fixed inset-0 z-[250] flex items-start justify-center p-4 pt-10 sm:p-6 sm:pt-16 bg-black/85 backdrop-blur-xl overflow-y-auto"
                        onClick={(e) => {
                    ...
                    >
                        <motion.div
                            ...
                            className="relative w-full max-w-md p-5 sm:p-8 rounded-3xl bg-[#0f1115] border border-white/15 shadow-[0_30px_100px_rgba(0,0,0,0.95),0_0_50px_rgba(249,115,22,0.15)]"
                        >
            ...
            {/* ═══ GOOGLE OAUTH SECURITY ALERT MODAL ('or is ka pop desigen kar lana ta ka user ko pata chla') ═══ */}
            <AnimatePresence>
                {showGoogleAlert && (
                    <motion.div
                        initial={{ opacity: 0 }}
                        ...
                        exit={{ opacity: 0 }}
                        transition={{ duration: 0.2 }}
                        className="fixed inset-0 z-[300] flex items-center justify-center p-4 sm:p-6 bg-black/90 backdrop-blur-2xl overflow-y-auto"
                        onClick={(e) => { if (e.target === e.currentTarget) setShowGoogleAlert(false); }}
                    >
                        <motion.div
                            ...
                            className="relative w-full max-w-md p-7 sm:p-8 rounded-3xl bg-gradient-to-b from-[#241712] via-[#161210] to-black border border-amber-500/30 shadow-[0_30px_100px_rgba(234,88,12,0.25)] text-center my-auto"
                        >
                            <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-amber-500/20 to-red-500/20 border border-amber-500/40 flex items-center justify-center mx-auto mb-5 shadow-lg text-2xl">
                                🔒
                            </div>
                            <h3 className="text-xl font-extrabold text-white tracking-tight mb-2">
                                Account Protected by Google OAuth
                            </h3>
                            <p className="text-xs sm:text-sm text-zinc-300 leading-relaxed mb-6">
                                The email address (<strong className="text-amber-400">{email || 'your account'}</strong>) is registered exclusively via <strong className="text-white font-bold">Google Sign-In</strong> and does not have a standalone password. For zero-trust enterprise security, credential login is blocked.
                            </p>
                            <div className="flex flex-col gap-3">
                                <button
                                    type="button"
                                    onClick={() => { setShowGoogleAlert(false); setShowLoginModal(false); signIn('google', { callbackUrl: '/' }); }}
                                    className="premium-btn-google py-3.5 text-sm font-extrabold shadow-xl justify-center w-full"
                                >
                                    <svg width="18" height="18" viewBox="0 0 24 24"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/></svg>
                                ...
                                <button
                                    ...
                                    className="py-3 px-4 rounded-xl text-xs font-bold text-zinc-400 hover:text-white bg-white/[0.05] hover:bg-white/[0.1] transition-all w-full"
                                >
                                    Cancel & Return
                                </button>
                            </div>
